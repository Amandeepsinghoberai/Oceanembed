...
export default function FisheriesModule() {
  ...
  if (!isLoading && ocean) {
    const { surface, subsurface, derived } = ocean;
    const depthIndex200 = subsurface.depth_m.indexOf(200);

    // Temperature
    if (surface.sst_c !== null) {
      tempDisplay = `${surface.sst_c.toFixed(1)} °C`;
      if (surface.sst_c >= 24.0 && surface.sst_c <= 28.5) tempScore = 30;
      else if (surface.sst_c > 28.5 && surface.sst_c <= 30.0) tempScore = 22;
      else if (surface.sst_c < 24.0 && surface.sst_c >= 20.0) tempScore = 20;
      else tempScore = 12;
    }
    ...
    if (surface.sss_psu !== null) {
      ...
      if (surface.sss_psu >= 34.2 && surface.sss_psu <= 35.8) salScore = 25;
      else if (surface.sss_psu < 34.2) salScore = 16;
      else salScore = 18;
    }
    ...
    if (derived.current_speed_ms !== null) {
      ...
      const spd = derived.current_speed_ms;
    ...
    // Profile / Subsurface
    if (subsurface.valid && depthIndex200 !== -1) {
      const surfTemp = subsurface.temperature_c[0];
      const tempAt200 = subsurface.temperature_c[depthIndex200];
      profDisplay = `0–200 m (T₂₀₀m: ${tempAt200.toFixed(1)} °C)`;

      const deltaT = surfTemp - tempAt200;
