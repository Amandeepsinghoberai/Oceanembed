...
export default function InteractiveIntelligenceExplorer() {
  // RealOceanMap owns its own "which point is active" concept — a known
  // demo location (selectedId) XOR a freehand map click (arbitraryPoint) —
  // rather than the single generic `selectedLocation` this file originally
  // assumed. Real, validated locations are loaded the same way the Solution
  // page loads them, via ModelResultsProvider.
  ...
  // Resolve whichever of the two is currently active into the single shape
  // the rest of this component (metrics, header) already expects.
  const knownLocation = locations.find((l) => l.id === selectedId) || null;
  const selectedLocation = arbitraryPoint
    ? { lat: arbitraryPoint.lat, lon: arbitraryPoint.lon, regionName: arbitraryPoint.region }
    : knownLocation
      ? { lat: knownLocation.lat, lon: knownLocation.lon, regionName: knownLocation.region }
      : { lat: 15.2, lon: 72.8, regionName: 'Arabian Sea' };
