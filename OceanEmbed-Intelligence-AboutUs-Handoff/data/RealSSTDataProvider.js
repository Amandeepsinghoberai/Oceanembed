// data/RealSSTDataProvider.js

class RealSSTDataProviderClass {
  constructor() {
    this.data = null;
    this.isLoaded = false;
    this._metadata = null;
  ...
  async load() {
    ...
    try {
      const mRes = await fetch('/data/sst/copernicus_2026-09-07.meta.json');
      if (!mRes.ok) throw new Error(`Metadata error! status: ${mRes.status}`);
      const meta = await mRes.json();

      const bRes = await fetch('/data/sst/copernicus_2026-09-07.bin');
      ...
      console.error("Failed to load regional SST data:", e);
    }
  }
  ...
  getSST(lat, lon, dateIndex = 0) {
    ...
    // Value is already Celsius
    return Math.round(val * 10) / 10;
  }

  async getSSTGrid(dateIndex = 0) {
    if (!this.isLoaded) return null;
    
    const latCount = this._metadata.latCount;
    const lonCount = this._metadata.lonCount;
    
    const grid = new Array(latCount);
    for (let i = 0; i < latCount; i++) {
      const row = new Array(lonCount);
      for (let j = 0; j < lonCount; j++) {
         let val = this._binaryData[(i * lonCount) + j];
  ...
  getGridInfo() {
    if (!this.isLoaded) return null;
    return {
      latitudeCount: this._latitude.length,
      longitudeCount: this._longitude.length,
      availableDates: this._availableDates
    };
  }
}

export const RealSSTDataProvider = new RealSSTDataProviderClass();
